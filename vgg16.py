...
class vgg16:
    ...
    def convlayers(self):
        self.parameters = [] #SM Create a tensor variable that is empty and has no shape yet.

        # zero-mean input
        with tf.name_scope('preprocess') as scope:
            mean = tf.constant([123.68, 116.779, 103.939], dtype=tf.float32, shape=[1, 1, 1, 3], name='img_mean')
            images = self.imgs-mean #SM: I think this manages to subtract the mean value from every pixel in
...
if __name__ == '__main__':
    sess = tf.Session()
    imgs = tf.placeholder(tf.float32, [None, 224, 224, 3]) #SM 4D tensor with the first dimension the image number
